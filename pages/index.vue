    ...
    <v-row align="start" justify="center">
      <v-col cols="12" sm="8" md="6">
        ...
        <v-flex row justify-end>
          <v-btn class="mx-1" fab dark floating small color="primary" @click="reloadData()">
            <v-icon class="lighten-1 white--text" v-text="'mdi-reload'"></v-icon>
          </v-btn>
        </v-flex>
        <v-list class="mt-3" two-line v-for="(list, listIndex) in lists" :key="listIndex">
          <v-subheader class="lighten-1 white--text">
            {{ list.header }}
            <span class="mx-1 lighten-1 primary--text">({{ list.tickets.length }})</span>
          </v-subheader>
          ...
          <v-list-item-group multiple active-class="secondary--text">
            <template v-for="(item, itemIndex) in list.tickets">
              <v-list-item :key="item.id" @click.stop="action(listIndex, item.event.id)">
                <v-list-item-avatar>
                  <v-icon
                    v-if="listIndex == 0"
                ...
                <v-list-item-action @click.stop="deleteAction(listIndex, item.id)">
                  <v-btn icon>
    ...
    <v-dialog v-model="dialog" max-width="290">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="green darken-1" text @click="dialog = false">{{ deleteCancel }}</v-btn>
          <v-btn color="green darken-1" text @click="deleteTicket()">{{ deleteOk }}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-container>
